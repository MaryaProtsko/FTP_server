...
void response(Command *cmd, State *state)
{
  switch(lookup_cmd(cmd->command)){
    ...
    case PASV:    
      state->message = "550 Please use PORT instead of PASSV.\n";
      write_state(state);
    //ftp_pasv(cmd,state); 
    break;
    ...
    case NOOP:
      if(state->logged_in){
        state->message = "200 NOOP.\n";
      }else{
        state->message = "530 Emty command.\n";
      }
      write_state(state);
      break;
    default: 
...
void ftp_port(Command *cmd, State *state)
{
   if(state->logged_in){
    ...
    char *response = "200 Entering Active Mode (%d,%d,%d,%d,%d,%d)\n";
    Port *port = malloc(sizeof(Port));  
  
    sscanf(cmd->arg,"%d,%d,%d,%d,%d,%d",&ip[0],&ip[1],&ip[2],&ip[3],&(port->p1),&(port->p2));
    sprintf(buff,"%d.%d.%d.%d",ip[0],ip[1],ip[2],ip[3]);
    
    // Connect to a client here 
    state->sock_actv = create_socket_act((256*port->p1)+port->p2,buff);
    printf("port: %d\n",256*port->p1+port->p2);
...
void ftp_pasv(Command *cmd, State *state)
{
    ...
    printf("Befor creation of random socet\n");
    printf("port: %d\n",256*port->p1+port->p2);
    //sprintf(buff,response,ip[0],ip[1],ip[2],ip[3],port->p1,port->p2);
    sprintf(buff,en_resp,ip[0],ip[1],ip[2],ip[3],port->p1,port->p2);
    state->message = buff;
    state->mode = SERVER;
    puts(state->message);
    write_state(state);
    state->sock_pasv = create_socket((256*port->p1)+port->p2);
    accept_connection(state->sock_pasv);
    sprintf(buff,"%d.%d.%d.%d",ip[0],ip[1],ip[2],ip[3]);
  ...
  if(state->logged_in==1){
    ...
    if(!dp){
      ...
      if(state->mode == SERVER){
        ...
        state->message = "226 Directory send OK.\n";
        state->mode = NORMAL;
        close(state->sock_actv); //да, его нужно закрывать - так клиент понимает, что передача закончилась
      }else{
...
void ftp_mkd(Command *cmd, State *state)
{
  if(state->logged_in){
    ...
    state->message = "500 Somthing complitly wrong!\n";
  }
  write_state(state);
}
...
void ftp_retr(Command *cmd, State *state)
{
  ...
  if(fork()==0){
    ...
    if(state->logged_in){
      ...
      if(state->mode == CLIENT){
        if(access(cmd->arg,R_OK)==0 && (fd = open(cmd->arg,O_RDONLY))){
          ...
          {
            printf("Is directory!\n");
            char *buf="tar cvfz %s.tar.gz %s\n";
            sprintf(buff,buf,cmd->arg,cmd->arg);
            printf("%s",buff);
            system(buff); 
            close(fd);
            strcat(cmd->arg,".tar.gz");
            fd = open(cmd->arg,O_RDONLY);
            fstat(fd,&stat_buf);
            state->message="150 Opening BINARY mode data connection (archived file).\n";
            is_archive=1;
          }
        
          write_state(state);
       
          if(sent_total = sendfile(state->sock_actv, fd, &offset, stat_buf.st_size)){
    ...
    close(fd);
    close(state->sock_actv);
    write_state(state);
    if(is_archive)
    {
      sprintf(buff,"rm %s",cmd->arg);
      system(buff);
    }
    exit(EXIT_SUCCESS);
  }
